    ...
    void dijkstra(ll source){
        ...
        while(!q.empty()){
            ...
            for(auto e : adjList[v]){
                //Relaxation
                if (distance[v] + abs(e.second) + weight[e.first] < distance[e.first] && distance[v] != inf){
                    distance[e.first] = distance[v] + abs(e.second) + weight[e.first];
                    parent[e.first] = v;
...
Graph load_graph(string filename){
    ...
    int nVertices, nEdges, u, v, w;
    file>>nVertices>>nEdges;

    Graph g(nVertices+1, false);
    string place, place1;
